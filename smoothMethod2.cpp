...
void disCheckMove(int i, vector<vertex> &v, vector<face> &f, double moveX, double moveY){
    ...
    if(!v[i].onBound){
        ...
        double ordis = findShortestDistInStar(v, f, v[i].x, v[i].y, i, &angleOrig);
        double dis = findShortestDistInStar(v, f, xTobeMove, yTobeMove, i, &angleToMove);
        if(((dis > ordis) || (dis == ordis)) && ((angleToMove > min_angel_glb) || (angleToMove == min_angel_glb)) ){
