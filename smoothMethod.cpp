...
int smoothLapAng(vector<vertex> &v, vector<face> &f){
    for (int i=0; i<v.size(); i++) {// for each vertex in mesh
    	...
    	if(v[i].onBound == 0){	
    		for(int j= 0; j<v[i].neighbors.size(); j++){ // for each vertex in neihgbor face
    			lastV = v[v[i].neighbors[(v[i].neighbors.size()+j-1)%v[i].neighbors.size()]];
                thisV = v[v[i].neighbors[j]];
                nextV = v[v[i].neighbors[(j+1)%v[i].neighbors.size()]];



                    //cout << "lastV thisV nextV " << v[i].neighbors[(v[i].neighbors.size()+j-1)%v[i].neighbors.size()]
                    //<<" " <<v[i].neighbors[j] <<" " <<v[i].neighbors[(j+1)%v[i].neighbors.size()] <<endl;
            ...
            v[i].x = xnew/v[i].neighbors.size();
            v[i].y = ynew/v[i].neighbors.size();
    ...
    for (int i=0; i<v.size(); i++) {// for each vertex in mesh
        ...
        if(v[i].onBound == 0){  
            ...
            for(int j= 0; j<v[i].numOfNeighborFace; j++){ // for each vertex in neihgbor face
                ...
                double ordis = findShortestDistInStarT(v, f, v[i].x, v[i].y, v[i].neighbors);
                double dis = findShortestDistInStarT(v, f, xTobeMove, yTobeMove, v[i].neighbors);
                if(dis < ordis){
                    xTobeMove = v[i].x;
                    yTobeMove = v[i].y;
                }
